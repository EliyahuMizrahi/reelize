...
Return ONLY valid JSON matching the provided schema."""
        ...
        for attempt in range(max_attempts):
            ...
            try:
                ...
                if not response.text:
                    self._log(f"⚠️ Empty response (attempt {attempt + 1}/{max_attempts})")
                ...
                self._log(f"❌ Error on {key_tag} (attempt {attempt + 1}/{max_attempts}): {e}")
                msg = str(e).lower()
                rate_limited = any(
                    t in msg for t in
                    ("429", "resource_exhausted", "quota", "rate limit")
                )
                if rate_limited and num_clients > 1:
                    self._client_idx = (self._client_idx + 1) % num_clients
                    self._log(f"   ↪ rotating to key {self._client_idx + 1}/{num_clients}")
    ...
    def _run_refinement_pass(self, coarse: ClipAnalysis, video_path: str,
        ...
        cfg = self.config
        long_segs = [s for s in coarse.segments
    ...
    def _refine_segment(self, seg: Segment, video_path: str,
                        game_detected: str, clip_context: str) -> list[Segment]:
        cfg = self.config
    ...
    def _verify_and_relabel_segments(self, segments, video_path, game_detected) -> int:
        ...
        for seg in segments:
            ...
            for i, t in enumerate(check_times):
                out_path = os.path.join(check_dir, f"check_{i}_{t:.2f}s.png")
                cmd = [
                    "ffmpeg", "-y", "-ss", str(t), "-i", video_path,
                    "-vframes", "1", "-vf", f"scale={cfg.frame_resize_width}:-1",
                    "-q:v", "2", out_path,
